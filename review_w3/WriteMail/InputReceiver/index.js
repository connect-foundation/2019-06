...
const ListOfReceivers = () => {
  ...
  const deleteByRegExp = regex => val =>
    val.replace(new RegExp(regex, "gi"), SC.BLANK);
  const replaceAndSetReceiver = (f, target) => {
    ...
    if (replaced !== SC.BLANK) {
      dispatch({
        ...
        payload: { receivers: [...receivers, replaced] }
      });
    }
    target.value = SC.BLANK;
    resizeInput(target);
  ...
  const keyDownHandler = e => {
    ...
    if (
      ...
      dispatch({
        type: UPDATE_RECEIVERS,
        payload: { receivers: receivers.slice(0, -1) }
      });
      resizeInput(target);
      ...
      <S.ReceiverListWrapper onClick={focusOn}>
        ...
        <S.ReceiverListUl>
          {receivers.map((receiver, idx) => (
            <S.ReceiverListLi
              validation={V.validate("email", receiver)}
              key={idx}
            >
              {receiver}
              <S.ReceiverLiDeleteBtn
                onClick={() => receiverDeleteBtn(receiver)}
              >
                X
              </S.ReceiverLiDeleteBtn>
            </S.ReceiverListLi>
